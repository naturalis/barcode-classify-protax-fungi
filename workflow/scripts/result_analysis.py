...
## query4.nameprob and query5.nameprob are used as these are the order and family levels until which the subtrees are classified.
...
def determine_query_file(test_true, query4_path, query5_path):
...
def main(args):
    ...
    query_path = determine_query_file(test_true, args.query4, args.query5)
    ...
    for _, row in df.iterrows():
        ...
        if lowest_true_level is not None:
            ...
            protax_probability = row["Probability"] if pd.notna(row["Probability"]) else 0
    ...
    overall_probability = results_df["Protax_Probability"].mean()
    ...
    with open(args.output, 'a') as f:
        f.write(f"\nTotal Accuracy,,,{overall_probability:.4f},{overall_accuracy:.4f}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--query4', help='Input: PROTAX classification until Order', required=True)
    parser.add_argument('--query5', help='Input: PROTAX classification until Family', required=True)
